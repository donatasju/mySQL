 ...
 * @return bool
 */
function validate_username($field_input, &$field, &$safe_input) {
    if (preg_match("/^[\w]+$/", $field_input)) {
        return true;
    }

    $field['error_msg'] = 'Invalid username, contains spaces or/and specials chars. (only "_" can be
        used as special char).';
}
